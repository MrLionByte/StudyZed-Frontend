    ...
    {students && Array.isArray(students) && students.length > 0 ? (
      ...
      {students.map((student) => (
        <div key={student.id} className={`aspect-video rounded-xl flex flex-col items-center p-4 ${
          ...
          {student.profile ?
          <img
            src={student.profile ? student.profile.profile_picture.replace("image/upload/", "") : ''}
          ...
          <p className="text-sm text-gray-400">First Name : {student.first_name}</p>
          <p className="text-sm text-gray-400">{student.email}</p>
          
          {(student.is_active) ? 
          <button className='rounded bg-slate-400 mt-4 p-2 cursor-pointer'
            onClick={(e)=> confirmationToast(e, student.id, false, student.username)}
            >BLOCK USER</button>
          :
          <button className='rounded bg-slate-400 mt-4 p-2 cursor-pointer hover:bg-green-400'
            onClick={(e)=> confirmationToast(e, student.id, true, student.username)}
            >UNBLOCK USER</button>
          }
        </div>
      ))}
