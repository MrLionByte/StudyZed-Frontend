  ...
  return (
    ...
    {sessions && Array.isArray(sessions) && sessions.length > 0 ? (
      ...
      {sessions.map((session) => (
        <div key={session.id} className={`aspect-video rounded-xl flex flex-col items-center p-4 bg-muted/50`} >
          
          <p className="font-bold">{session.duration} month{session.duration > 1 ? "s" : ""}</p>
          <p className="text-sm text-gray-400">Price : {session.amount}</p>
