...
const StudentTask = () => {
  ...
  const getTaskStatus = (taskDate) => {
    ...
    const taskDueUTC = new Date(
      Date.UTC(
        taskDue.getUTCFullYear(),
        taskDue.getUTCMonth(),
        taskDue.getUTCDate(),
      ),
    );
    ...
    >
      <div className="p-5">
        <div className="flex items-start justify-between mb-4">
          ...
          <div
            className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(status)}`}
          >
        ...
        <div className="space-y-4">
          <p className="text-gray-50 text-sm break-words">
            {task.description}
          </p>
  ...
  const groupedTasks = {
    today: sortedTasks.filter(
      (task) => getTaskStatus(task.due_date) === 'today',
    ),
    upcoming: sortedTasks.filter(
      (task) => getTaskStatus(task.due_date) === 'upcoming',
    ),
