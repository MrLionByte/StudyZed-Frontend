...
const AttendAssessmentModal = ({ assessmentData, handleClose, onSubmit = () => {} }) => {
  ...
  const updateAnswer = (questionId, value, questionType) => {
    setAnswers(prev => 
      prev.map(ans => 
        ans.questionId === questionId ? { ...ans, answer: value, questionType:questionType } : ans
  ...
  const renderQuestionInput = (question) => {
    ...
    switch(questionType) {
      case 'OPEN':
        return (
          <Textarea 
            ...
            onChange={(e) => updateAnswer(question.id, e.target.value, questionType)}
      ...
      case 'SINGLE_CHOICE':
        return (
          <RadioGroup 
            onValueChange={(value) => updateAnswer(question.id, value, questionType)}
