...
import { User, Users, Video, Phone, X,Calendar,Clock,CalendarCheck,CalendarCog } from "lucide-react";
...
const SessionSelection = () => {
  ...
  const handleEndInBackend = async (groupSession, status) =>{
    ...
    try{
      const response =await api.patch('meet/change-session-status/',
        payload,{
        baseURL: url
      })
      console.log(response);
      ...
      {groupSession ? (
        ...
        >
          ...
          <div className="flex items-center space-x-4 mb-3">
            ...
            <p className="text-sm">{groupSession.started_at.slice(0,10)}</p>
          </div>

          <div className="flex items-center space-x-4 mb-4">
            <Clock className="h-5 w-5 text-emerald-400" />
            <p className="text-sm">{groupSession.started_at.slice(11,16)}</p>
          </div>
        ...
        <div
          ...
          rounded-lg w-full"
        >
          <h1 className="text-center font-bold text-red-400">No Session Scheduled</h1>
        </div>
      )}

      {groupSession.status == 'ended' || groupSession == '' &&
      <button 
    ...
    <div className="flex flex-col p-6 border-2 border-emerald-500 rounded-lg w-full">
      <form onSubmit={handleScheduleSession}>
        ...
        <div className="flex justify-center mt-5">
          <button type="submit" className="rounded-full p-2 bg-emerald-600 font-bold hover:bg-blue-500">
            Schedule Now
          </button>
        </div>
      </form>
