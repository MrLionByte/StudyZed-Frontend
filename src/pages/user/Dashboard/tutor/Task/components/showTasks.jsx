...
const TaskSubmittedModal = ({ taskData, handleClose }) => {
  ...
  const handleConfirmMark = () => {
    ...
    try{
      ...
      const response = api.patch(`task-tutor/tasks/${taskData.id}/mark/`,
        {score:mark},
        {
        baseURL: url,
      })
          ...
          {taskData ? (
            <div className="flex flex-col space-y-2">
              <div className="flex justify-between rounded-lg">
                <h3 className="text-lg text-white font-bold break-words">
                  {taskData.id}
                </h3>
                {taskData.score = null || !markMarked ?
        ...
        <Dialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
          <DialogContent className="text-black bg-slate-300 bg-opacity-50 sm:max-w-md">
            ...
            <DialogFooter className="gap-4 sm:justify-end">
              <Button className="bg-slate-300 hover:bg-emerald-400"
                type="button"
                variant="primary"
              ...
              <DialogClose asChild>
                <Button type="button" className="text-black hover:bg-red-400"
                variant="secondary">
                  No
                </Button>
              </DialogClose>
