...
const StudentPerformanceChart = ({ assessmentPerformance = [] }) => {
  ...
  const [chartData, setChartData] = useState({
    ...
    options: {
      ...
      colors : [
        "#FF4560", "#008FFB", "#00E396", "#775DD0", "#FEB019",
        "#FF6384", "#36A2EB", "#4BC0C0", "#9966FF", "#FF9F40",
        "#FFCD56", "#4D4D4D", "#F7464A", "#46BFBD", "#FDB45C",
        "#949FB1", "#8A2BE2", "#A52A2A", "#DEB887", "#5F9EA0",
        ...
        "#696969", "#1E90FF", "#B22222", "#228B22", "#FFD700",
        "#DAA520", "#808080", "#008000", "#ADFF2F", "#FF69B4",
        "#CD5C5C", "#4B0082", "#F0E68C", "#E6E6FA", "#7CFC00",
  ...
  useEffect(() => {
    if (assessmentPerformance.data && assessmentPerformance.data.length > 0) {
      ...
      const uniqueAssessments = [...new Set(assessmentPerformance.data.flatMap(student => student.data.map((_, index) => `Assessment ${index + 1}`)))];
    ...
    <div className="p-4 bg-gray-300 bg-opacity-80 text-white shadow-md rounded-lg">
      <div className="flex items-center justify-start">
        <button onClick={handleLastMonth}
          className="text-black mr-2 hover:text-red-500"><LucideSquareArrowLeft /></button>
        <h5 className="text-lg text-black " >month of 
          <span className="text-black font-semibold ml-2">{Months[assessmentPerformance.month]}-{assessmentPerformance.year}</span></h5>
      </div>
      <h2 className="text-lg font-semibold text-center">Assessment Performance</h2>
