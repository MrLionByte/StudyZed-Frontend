...
export default function Assessment({session_data}) {
  const { assessments, loading, isCreatingAssessment, scheduled, ongoing, completed,
    setIsCreatingAssessment, setFetchFromBackend } = useAssessments();
    ...
    return (
        ...
        <div className="flex-1 space-y-8 ">
          ...
          {/* <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-black/20 backdrop-blur-sm border border-teal-800/30 rounded-lg p-6">
              <h3 className="text-teal-400 font-bold mb-2">SCHEDULED ASSESSMENT {scheduled}</h3>
            </div>
            <div className="bg-black/20 backdrop-blur-sm border border-teal-800/30 rounded-lg p-6">
              <h3 className="text-teal-400 font-bold mb-2">ONGOING ASSESSMENT 
                <span className="text-red-500 pl-2">{ongoing}</span></h3>
            </div>
            <div className="bg-black/20 backdrop-blur-sm border border-teal-800/30 rounded-lg p-6">
              <h3 className="text-teal-400 font-bold mb-2">COMPLETED ASSESSMENT {completed}</h3>
            </div>
          </div> */}
      ...
      {isCreatingAssessment && (
        < CreateAssessment handleClose={closeModal} fetchFromBackend={fetchFromBackend} />
      )}
