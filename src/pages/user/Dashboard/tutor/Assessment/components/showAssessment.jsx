...
const AssessmentModal = ({ assessmentData, handleClose, onSave }) => {
  ...
  const addOption = (questionIndex) => {
    ...
    const newOptionNo = currentOptions.length > 0 
        ...
        <div className="p-6 space-y-4">
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-bold text-teal-400">
              {assessmentData ? (isEditing ? 'Edit Assessment' : 'Assessment Details') : 'Create Assessment'}
            </h2>
              ...
              <div>
                ...
                <div className="space-y-6">
                  {formData.questions.map((question, qIndex) => (
                    <div key={question.id || qIndex} className="bg-black/20 p-4 rounded-lg relative">
                      <button
                        type="button"
                      ...
                      <div className="space-y-4 mt-2">
                        <div>
                          <label className="block text-gray-300 mb-1">Question {qIndex + 1}</label>
                          <input
                            type="text"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            <label className="block text-gray-300 mb-1">Question Type</label>
                            <select
                        ...
                        {question.question_type !== 'OPEN' && (
                          <div className="mt-4">
                            <div className="flex justify-between items-center mb-2">
                              <label className="text-gray-300 font-semibold">Options</label>
                              <button
                                type="button"
                            ...
                            {question.options?.map((option, optIndex) => (
                              <div key={option.option_no} className="bg-black/30 p-3 rounded-lg mb-2 relative">
                                <button
                                  type="button"
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-2 items-center">
                                  <div className="md:col-span-3">
                                    <label className="text-xs text-gray-400 mb-1 block">Option {option.option_no}</label>
                                    <input
                                      type="text"
                                      value={option.option}
                                      onChange={(e) => handleOptionChange(qIndex, optIndex, 'option', e.target.value)}
                                      className="w-full bg-gray-800 border border-gray-700 rounded-md p-2 text-white"
                                      required
                                    />
                                  </div>
                                  ...
                                  <div>
                                    <label className="text-xs text-gray-400 mb-1 block">Correct?</label>
                                    <select
                                      value={option.is_correct.toString()}
                                      onChange={(e) => handleOptionChange(qIndex, optIndex, 'is_correct', e.target.value)}
                                      className="w-full bg-gray-800 border border-gray-700 rounded-md p-2 text-white"
                                    >
                            ...
                            {question.options?.length < 2 && (
                              <p className="text-red-400 text-sm">At least 2 options are required.</p>
                            )}
                          </div>
                        )}
                      </div>
                  ...
                  {formData.questions.length === 0 && (
                    <div className="text-center py-6 bg-black/20 rounded-lg">
                      <p className="text-gray-400">No questions yet. Click "Add Question" to begin.</p>
                    </div>
                  )}
                </div>
              </div>
