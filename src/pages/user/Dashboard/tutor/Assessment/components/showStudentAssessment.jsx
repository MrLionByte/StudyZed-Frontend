...
const StudentAssessmentModal = ({ assessmentData, handleClose }) => {
  ...
  const handleSubmitFinialMark = async () => {
    try {
      ...
      const response = await api.patch(
        TutorEndPoints.UpdateAssessmentMark,
        tempMarks,
        {
          baseURL: url,
        },
  ...
  return (
    ...
    >
      <div
        className="bg-gray-900/95 border border-teal-800/30 rounded-lg 
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="space-y-6">
            ...
            Status: {assessment.is_completed ? "Completed" : "Incomplete"}
                ...
                <p className="text-gray-400">
                  Late Submission:{' '}
                  {assessment.is_late_submission ? 'Yes' : 'No'}
                </p>
                <p className="text-gray-400">
                ...
                <p className="text-gray-400">
                  Completed:{' '}
                  {new Date(assessment.completed_on).toLocaleString()}
                </p>
                      ...
                      {response.question.question_type === 'OPEN' && (
                        <div className="mt-2 flex justify-between">
                          <div>
                            ...
                            <p className="text-white">
                              {response.open_response || 'No response provided'}
                            </p>
                          </div>
                          <div>
                            {response?.mark ? (
          ...
          <div className="flex justify-end">
            <button
              className="text-center bg-emerald-400 hover:bg-emerald-700 hover:text-emerald-300 p-2 rounded-full
        font-semibold"
              onClick={handleSubmitFinialMark}
            >
              FINALIZE MARK
            </button>
          </div>
