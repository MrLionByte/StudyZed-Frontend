...
import { MessageSquare, Send, User, LayoutDashboard, ClipboardList, 
  ListTodo, Users, Activity, UserCircle } from 'lucide-react';
...
const ChatComponent = ({ chatId, currentUser, session_data }) => {
  ...
  useEffect(() => {
    const fetchChattedUsers = async () => {
      ...
      try {
        ...
        const url = api_dictatory["Message_Service"]

        const response = await api.get(`chatted-users/${user_data.user_code}/`, {
            baseURL: url
          });
  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      // Use Django's default port
      console.log("CONNECt :",selectedUser, selectedUser_ID);
      const chatId = selectedUser_ID || 1999 //recipient_id
      const ws = new 
        WebSocket(`ws://localhost:8006/ws/chat/${chatId}/?user_id=${user_data.user.id}`);
  ...
  const handleSelectedUser = (user_code, user_id) => {
    if (user_code ){
      const selected = chattedUsers.find(user => user.user_code === user_code);
  ...
  const mockMessages = {
    1: [
      { id: 1, userId: 1, text: "Hi, I need help with React hooks", isTutor: false, timestamp: "10:30 AM" },
      { id: 2, userId: 1, text: "Specifically, useEffect is confusing", isTutor: false, timestamp: "10:31 AM" },
      { id: 3, userId: 1, text: "I'll explain how useEffect works with examples", isTutor: true, timestamp: "10:35 AM" },
    ],
    2: [
      { id: 1, userId: 2, text: "Your explanation was very helpful", isTutor: false, timestamp: "9:20 AM" },
      { id: 2, userId: 2, text: "Glad I could help! Let me know if you have more questions", isTutor: true, timestamp: "9:25 AM" },
    ],
  };
          ...
          {allStudentsMessages.map((user) => (
            <div
              ...
              className={`p-4 border-b border-gray-800 flex items-center gap-3 cursor-pointer hover:bg-gray-800 transition-colors ${
                selectedUser?.user_code === user.user_code ? "bg-gray-800" : ""
              }`}
            >
              <img
                src={(user.profile?.profile_picture ? user.profile?.profile_picture?.replace("image/upload/", ""):
                      ...
                      >
                        ...
                        <p
                          className={`text-xs mt-1 ${
                            message.isTutor ? "text-emerald-100" : "text-gray-400"
                          }`}
                        >
                          {message.timestamp}
                        </p>
                ...
                : selectedUser.latest_message && (
                    ...
                    >
                      ...
                      >
                        ...
                        <p
                          className={`text-xs mt-1 ${
                            selectedUser.latest_message.isTutor ? "text-emerald-100" : "text-gray-400"
                          }`}
                        >
